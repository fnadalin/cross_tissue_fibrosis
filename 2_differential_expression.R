...
runMiloDE <- function(q_obj, latent_id, out_miloDE_prefix) {
    de_results_file <- paste0(out_miloDE_prefix, ".tsv")
    differential_expression_miloDE(q_obj, latent_id = latent_id, out_miloDE_prefix)
}

annMiloDE <- function(out_miloDE_prefix, q_lab_soft, r_name, meta) {
    sce_file <- paste0(out_miloDE_prefix, ".Rds")
    de_results_file <- paste0(out_miloDE_prefix, ".tsv")
    de.res <- annotate_neighbourhoods_soft_miloDE(sce_file, de_results_file, q_lab_soft)
    de_results_ann_file <- paste0(out_miloDE_prefix, "_", r_name, "_", meta, ".tsv")
    write_annotate_neighbourhoods_soft_miloDE(de.res, de_results_ann_file)
}
...
library("funr", lib.loc = "/hps/software/users/marioni/francesca/R_libs")

WORKING_DIR <- getwd()
SCRIPT_PATH <- dirname(sys.script())
SCRIPT_NAME <- basename(sys.script())
setwd(SCRIPT_PATH)
source(file.path(DIR, "differential_expression.R"))
...
for (m in meta) {
    ...
    q_lab_soft <- q_lab[2]
    annMiloDE(out_miloDE_scvi_prefix, q_lab_soft, r_name, m)
}

# scANVI
for (m in meta) {
    cat(paste0("Run miloDE on scANVI ", m, "\n"))
    q_milo <- create_scanvi_mod_query(q_milo_dir, r_name, m) 
    out_miloDE_scanvi_prefix <- paste0(q_milo, "_miloDE")
    latent_id <- paste0("X_scanvi_", r_name, "_", m)
    runMiloDE(q_obj, latent_id = latent_id, out_miloDE_scanvi_prefix)
    q_lab <- create_labels_file(q_obj_dir, r_name, m)
    q_lab_soft <- q_lab[2]
    annMiloDE(out_miloDE_scanvi_prefix, q_lab_soft, r_name, m)
}
